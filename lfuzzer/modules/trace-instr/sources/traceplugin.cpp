...
public:
        ...
        , M(M)
    {
        ...
        additionalOperators["globalPrint"]     = new foperator(GLOBPRINT,  false, false);
        ...
        additionalOperators["strncasecmp"]     = new foperator(STRCMP,       false, true);
     ...
     */
    void printFunctionCall(IRBuilder<>* builder, Instruction* instr, std::string& calledFunctionName)
    {
        ...
        // NOTE: We don't close the event with instructionEnd() if a tracer function is called because
        //       it might be a call to a wrapped function. The wrapper
        //       will want to add extra data to this call event.

        if (calledFunctionName.rfind("tracerllvm_", 0) != 0) {
            builderMutex.lock();
    ...
    void createFileHandlingCalls(IRBuilder<>* builder, Instruction* instr, const std::string& funNameString, bool printStrings)
    {
        ...
        for (unsigned int x = 0; x < instr->getNumOperands(); ++x)
        {
            ...
            if (printStrings && pTy != nullptr && pTy->getElementType()->isIntegerTy() && pTy->getElementType()->getPrimitiveSizeInBits() == 8)
            {
                ...
                else if (funNameString == "strncmp" || funNameString == "strncasecmp")
                {
    ...
    void instrumentInstr(Instruction* instr, IRBuilder<>* builder, IRBuilder<>* nextInstructionBuilder)
    {
        ...
        if (CallInst* callinst = dyn_cast<CallInst>(instr))
        {
            ...
            printFunctionCall(builder, instr, calledFunctionName);
        }
        else
        {
            std::vector<Value*> args;
    ...
    bool instrumentFunction(Function& F)
    {
        ...
        if (F.getName() != "_real_program_main")
        {
            ...
            // create calls to print
            Instruction* inst = &*(inst_begin(F));
            IRBuilder<> builder(inst);

            auto funName = getGlobalString(&builder, "_real_program_main");
            auto ret = getGlobalString(&builder, "void");

            builderMutex.lock();
            builder.CreateCall(tracer("instructionHeader"), {builder.getInt64(additionalOperators.at("globalPrint")->opcode), funName, ret, ret});
            for (llvm::Module::global_iterator ii = M.global_begin(); ii != M.global_end(); ++ii) {
                GlobalVariable* gv = &*ii;
                // check if a name is present and report it if so
                if (gv->getName().data()[0] != '\0') {
//                 write for each global variable the needed information
                    std::vector<Value*> args;
                    builderMutex.unlock();
                    fillArrayForOperand(&builder, args, gv);
                    builderMutex.lock();
                    builder.CreateCall(tracer("callOperandInt"), args);
                }
            }
