...
public class ReturnInst extends Operation {
    ...
    public void handleToken(NodeMapper nodeMapper, TokenManager tokenManager, EventSender eventSender) {
        ...
        if (getOperands().length > 0 && "Constant".equals(getOperands()[0].getName()) && "i32".equals(getOperands()[0].getType())) {
            tokenManager.getValue().ifPresent(val -> tokenManager.getTnt().ifPresent(tnt -> nodeMapper.addTaintForLocal(newNode, tnt)));
        }
        tokenManager.clean();
    }
}
