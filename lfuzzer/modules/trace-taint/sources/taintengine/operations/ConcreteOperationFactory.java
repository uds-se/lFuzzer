...
public class ConcreteOperationFactory implements OperationFactory {
    ...
    public Operation call(LineInformation info) {
        ...
        if (instruction.contains("@llvm.memmove") || instruction.contains("@memmove")) {
            return new MemCopy(info);
        }
        ...
        if (instruction.contains("@isdigit(")) {
            return new IsDigit(info);
        }
        if (instruction.contains("@toupper(")) {
            return new GenericOperation(info);
        }
        if (instruction.contains("@__assert_rtn")) {
            return new Assert(info);
        }
    ...
    @Override
    public Operation globalPrint(LineInformation info) {
        return new GlobalPrint(info);
    }
