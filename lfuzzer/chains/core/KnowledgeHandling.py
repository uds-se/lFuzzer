...
class KnowledgeHandling():
    ...
    # the stack size on the last successful token compare
    last_tokencomp_stack = 0
    ...
    def _extractStackEvents(self, objs) -> Tuple[List[Union[int, Tuple[Any, int]]], List[Union[float, Any]]]:
        ...
        for val in objs:
            if val["type"] == "STACK_EVENT":
                ...
                if index > lastindex:
                    lastindex = index
                if val["operator"] == "tokencomp" and val["value"] == val["operand"][0]:
                    self.last_tokencomp_stack = len(val["stack"])
    ...
    def __str__(self):
        return str(self.last_tokencomp_stack)
